...
def main():
    ...
    with col2:
        with col2:
            ...
            st.markdown(
                "<h3 style='text-align: center; color: #ff008c;'>Your personal AI<br/>synchronisation assistant.</h3>",
                unsafe_allow_html=True,
            )
    with col3:
    ...
    if uploaded_video is not None:
        ...
        if (
            "video" not in st.session_state
            or uploaded_video.name != st.session_state.video
        ):
        ...
        with st.sidebar:
            dancers = st.number_input(
                "Number of dancers (1-6):", value=2, min_value=1, max_value=6
            )
        ...
        if st.sidebar.checkbox(
            "Click to start (RESET this checkbox to upload new video)"
        ):
            ...
            def create_fig(x, y, title, hover_name, labels):
                fig = px.line(
                    df, x=x, y=y, title=title, hover_name=hover_name, labels=labels
                )
            ...
            fig0 = create_fig(
                ...
                "frames",
                {"frames": "Frame Number", "Smoothed_error": "Sync Error (degrees)"},
            )
            ...
            with st.expander("**View freeze frames:**"):
                frame = st.number_input(
                    "View frames starting from choice",
                    value=0,
                    min_value=0,
