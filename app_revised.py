...
def display_dial(title, value, color):
    st.markdown(
        div(
            ...
            big(style=styles(font_size=rem(2), font_weight=400, line_height=1))(value),
        ),
...
def processing(stats):
    ...
    # Create df
    d = {"Time": response["timestamps"], "Sync Error": response["scores"]}
    df = pd.DataFrame(d)
    ...
    st.write("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    st.write(
        "The following graph shows your absolute synchronisation error, calculated as a difference in pose/angle of major joints relative to each other. A lower value is a smaller difference, and therefore better synchronization. Peaks are areas of lowest synchronisation."
    )
    ...
    st.write("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    st.write(
        "The following graph shows your average synchronisation error, smoothed for easier reference of intervals needing improvement or attention."
    )
    ...
    # Timestamp buttons
    sorted_df = (
        df.sort_values(by=["Smoothed Sync Error"], ascending=False).round(2).head(5)
    )
    ...
    with st.expander("**Your top five areas for improvement:**"):
        for i in range(len(index)):
            ...
            st.image(
                f"https://storage.googleapis.com/sync_testinput/screencaps/{response['my_uuid']}/frame{top5imp[i]}.jpg"
            )
            st.write("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
...
def main():
    ...
    with col2:
        st.markdown(
            "<h1 style='text-align: center; color: yellow;'>Dance Synchronisation</h1>",
            unsafe_allow_html=True,
        )
    with col3:
    ...
    uploaded_video = st.file_uploader(
        "**Upload video for evaluation**", ["mp4"], key="dance"
    )
    ...
    if uploaded_video is not None:
        ...
        with st_lottie_spinner(lottie_model_loading):
            ...
            files = {
                "file": (uploaded_video.name, uploaded_video, "multipart/form-data")
            }
        ...
        with a:
            dancers = st.number_input(
                "Enter number of dancers:", min_value=1, max_value=6
            )
        with b:
            if st.button("Start"):
                show_vid.empty()
